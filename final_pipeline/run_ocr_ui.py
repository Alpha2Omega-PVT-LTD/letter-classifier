...
# ─────────────────────────────────────────────────────────
# System Path Setup (Self-Contained inside final_pipeline)
# ─────────────────────────────────────────────────────────
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
FINAL_PIPELINE_DIR = BASE_DIR
SLMS_DIR = os.path.join(BASE_DIR, "slms", "slms")
...
# ─────────────────────────────────────────────────────────
# ─────────────────────────────────────────────────────────
# 3-Model Cross-Checking & Evidence-Based Validation Pipeline
# ─────────────────────────────────────────────────────────
...
def check_ocr_evidence(entity_text: str, clinical_text: str) -> tuple:
    """
    Validates whether entity_text exists or is supported in the clean OCR text.
    Handles colons, punctuation variations, numbers, and unit formatting (e.g. Weight: 97.6 kg, BMI 36.28).
    Returns (is_supported, matched_sentence).
    """
    ...
    # 2. Normalized Punctuation & Whitespace Match
    ...
    # 3. Flexible Token Overlap (Includes numbers, short vitals like BP, HR, BMI, Weight, kg, mg)
    ...
    if tokens:
        ...
        # Check numbers if present in entity
...
def analyze_entity_context(entity_text: str, sentence_context: str, category: str) -> str:
    """
    Determines clinical context/status: Current, Historical, Suspected, Planned, Negated, Family History.
    """
    s_lower = sentence_context.lower() if sentence_context else ""
    
    # 1. Patient vs Other Person / Family History
...
def validate_and_correct_category(entity_text: str, candidate_categories: List[str]) -> str:
    """
    Overrules invalid model predictions using strict semantic category guardrails.
    NEVER allows chronic back pain -> Medication, physiotherapy -> Diagnosis, etc.
    """
    text_lower = entity_text.lower().strip()

    # 1. Procedure Keywords (High Priority for Interventions/Therapies/Referrals)
    procedure_kws = [
    ...
    # 2. Symptom Keywords (High Priority for Complaints/Signs)
    symptom_kws = [
    ...
    # 3. Vital / Lab Keywords & Measurements
    vital_kws = [
    ...
    vital_abbr_pattern = re.compile(r'\b(bp|hr|rr|wt|ht|hb|crp|esr|wbc|bmi)\b', re.IGNORECASE)
    if vital_abbr_pattern.search(text_lower) or any(k in text_lower for k in vital_kws) or re.search(r'\b\d{2,3}/\d{2,3}\b', text_lower):
        if 'weight loss' not in text_lower and 'weight gain' not in text_lower:
            return "Vital"

    # 4. Explicit Diagnosis Keywords
    diag_kws = [
    ...
    # 5. Medication (Strict check on drug names & specific suffixes)
    med_keywords = [
    ...
    # Fallback to majority vote among candidate categories
...
def build_3_model_consensus_from_final_pipeline(cb_cats: dict, mc_cats: dict, qw_cats: dict, clinical_text: str = ""):
    ...
    # 1. Collect ClinicalBERT candidates
    ...
    # Step 4b: Qwen Dynamic LLM Junk & Noise Filter
    merged_pool = qwen_filter_junk_entities(merged_pool, clinical_text)

    # 5. Validation, Context Analysis, and Confidence Layer
    all_entities = []
    counter = 0
    ...
    for item in merged_pool:
        ...
        # Step 5a: OCR Evidence Verification (Hallucination Prevention)
        ...
        # Step 5b: Deterministic Semantic Category Validation
...
# ─────────────────────────────────────────────────────────
app = FastAPI(title="NHS PDF Letter OCR & Coding Platform")

SESSION_FILE = os.path.join(BASE_DIR, ".session_ocr_pdf_hub.json")
...
# HTTP Middleware to disable static asset caching
@app.middleware("http")
...
class EntityDecisionItem(BaseModel):
    id: str
    text: str
    category: str
    ...
    snomed: Optional[str] = ""
    decision: Optional[str] = "Yes"

class ProcessRowRequest(BaseModel):
    row_index: int
    decisions: List[EntityDecisionItem]
...
def get_default_folder():
    ...
    folder = session.get("folder_path", "")
    if not folder or not os.path.exists(folder):
        default_sample = os.path.abspath(os.path.join(BASE_DIR, "sample_letters"))
        if os.path.exists(default_sample):
            folder = default_sample
    return {"default_folder": folder}

@app.post("/api/load-folder")
def load_pdf_folder(request: LoadFolderRequest):
    ...
    # List all PDF files inside selected folder
    pdf_files = sorted([
    ...
    if not pdf_files:
        raise HTTPException(status_code=400, detail="No PDF or text letters found in folder.")
    ...
    new_records = [
        {
            ...
            "has_extractions": r["has_extractions"],
            "reviewed": r["reviewed"]
        } for r in records
    ]
